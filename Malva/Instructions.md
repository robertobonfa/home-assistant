# Malva — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro rosa-malva per Home Assistant, derivato dalla palette **Malva** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dalla prugna scura al rosa cipria, piu quattro accenti
semantici scelti per non confondersi con i rosati della rampa.

A mauve-pink dark theme for Home Assistant, derived from the **Malva**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from dark plum to powder pink, plus four semantic accents
picked not to blend into the palette's own pinks.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| M0 | `#2b1d28` | sfondo app, header, sidebar | app background, header, sidebar |
| M1 | `#493344` | superficie delle card | card surface |
| M2 | `#684a5f` | bordi, divider, disabilitati | borders, dividers, disabled |
| M3 | `#88627b` | accent, slider, selezioni | accent, sliders, selections |
| M4 | `#a67f95` | colore primario, icone attive | primary colour, active icons |
| M5 | `#c19fae` | hover, stati attivi | hover, active states |
| M6 | `#dcc4cc` | testo secondario | secondary text |
| M7 | `#f7edef` | testo primario | primary text |

**Semantici / Semantic** — `#d94452` errore·error · `#ddaa55` avviso·warning ·
`#74b285` successo·success · `#eac36a` luci accese·lights on
...
```
Malva/
├── malva.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── malva-bg.svg                  # sorgente vettoriale / vector source
    ├── malva-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── malva-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `malva` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/malva/malva-bg-2560x1440.png
         homeassistant/www/malva/malva-bg-1170x2532.png

altre  ▸  config/www/malva/malva-bg-2560x1440.png
          config/www/malva/malva-bg-1170x2532.png
```

> La sottocartella `malva` non è facoltativa: il tema punta a
> `/local/malva/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `malva/` dalla
> riga `lovelace-background` in `malva.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `malva` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `malva`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `malva.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #261a23 0%, #2b1d28 45%, #402c3c 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/malva.yaml
altre  ▸  config/themes/malva.yaml
```
...
contenuto di `malva.yaml` e salva con l'icona del dischetto.
...
`malva.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Malva**.

> Se **Malva** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/malva.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `malva.yaml` sostituendo il nome
del file con `malva-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/malva/malva-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/malva/` dentro la cartella di configurazione.
...
create a `malva` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/malva/malva-bg-2560x1440.png
          homeassistant/www/malva/malva-bg-1170x2532.png

others ▸  config/www/malva/malva-bg-2560x1440.png
          config/www/malva/malva-bg-1170x2532.png
```

> The `malva` subfolder is not optional: the theme points at `/local/malva/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `malva/` from the
> `lovelace-background` line in `malva.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`malva` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `malva`, then drag the PNGs in.
...
The theme works without the image. Open `malva.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/malva.yaml
others ▸  config/themes/malva.yaml
```
...
`malva.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `malva.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Malva**.

> If **Malva** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/malva.yaml` and not in the config root;
...
edit the `lovelace-background` line in `malva.yaml` and swap the filename for
`malva-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/malva/malva-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/malva/` inside the configuration folder.
