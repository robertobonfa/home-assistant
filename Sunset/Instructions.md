# Sunset — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro caldo per Home Assistant, derivato dalla palette **Sunset** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal viola crepuscolare al giallo sabbia, piu quattro accenti
semantici scelti per non confondersi con i caldi della rampa.

A warm dark theme for Home Assistant, derived from the **Sunset**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from twilight purple to sand yellow, plus four semantic
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| S0 | `#1d1135` | sfondo app, header, sidebar | app background, header, sidebar |
| S1 | `#4b1d5e` | superficie delle card | card surface |
| S2 | `#883677` | bordi, divider, disabilitati | borders, dividers, disabled |
| S3 | `#c94b7c` | accent, slider, selezioni | accent, sliders, selections |
| S4 | `#ee6c6b` | colore primario, icone attive | primary colour, active icons |
| S5 | `#ff9770` | hover, stati attivi | hover, active states |
| S6 | `#ffbe73` | testo secondario | secondary text |
| S7 | `#ffe8b0` | testo primario | primary text |

**Semantici / Semantic** — `#e01e37` errore·error · `#ffb703` avviso·warning ·
`#3fb98a` successo·success · `#ffd166` luci accese·lights on
...
```
Sunset/
├── sunset.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── sunset-bg.svg                  # sorgente vettoriale / vector source
    ├── sunset-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── sunset-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `sunset` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/sunset/sunset-bg-2560x1440.png
         homeassistant/www/sunset/sunset-bg-1170x2532.png

altre  ▸  config/www/sunset/sunset-bg-2560x1440.png
          config/www/sunset/sunset-bg-1170x2532.png
```

> La sottocartella `sunset` non è facoltativa: il tema punta a
> `/local/sunset/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `sunset/` dalla
> riga `lovelace-background` in `sunset.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `sunset` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `sunset`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `sunset.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #1a0f2f 0%, #1d1135 45%, #3d1952 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/sunset.yaml
altre  ▸  config/themes/sunset.yaml
```
...
contenuto di `sunset.yaml` e salva con l'icona del dischetto.
...
`sunset.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Sunset**.

> Se **Sunset** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/sunset.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `sunset.yaml` sostituendo il nome
del file con `sunset-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/sunset/sunset-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/sunset/` dentro la cartella di configurazione.
...
create a `sunset` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/sunset/sunset-bg-2560x1440.png
          homeassistant/www/sunset/sunset-bg-1170x2532.png

others ▸  config/www/sunset/sunset-bg-2560x1440.png
          config/www/sunset/sunset-bg-1170x2532.png
```

> The `sunset` subfolder is not optional: the theme points at `/local/sunset/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `sunset/` from the
> `lovelace-background` line in `sunset.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`sunset` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `sunset`, then drag the PNGs in.
...
The theme works without the image. Open `sunset.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/sunset.yaml
others ▸  config/themes/sunset.yaml
```
...
`sunset.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `sunset.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Sunset**.

> If **Sunset** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/sunset.yaml` and not in the config root;
...
edit the `lovelace-background` line in `sunset.yaml` and swap the filename for
`sunset-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/sunset/sunset-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/sunset/` inside the configuration folder.
