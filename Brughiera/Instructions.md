# Brughiera — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro viola-grigio per Home Assistant, derivato dalla palette **Brughiera** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dall'erica notturna al lilla polvere, piu quattro accenti
semantici scelti per emergere da una rampa volutamente smorzata.

A heather-grey dark theme for Home Assistant, derived from the **Brughiera**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from night heather to dusty lilac, plus four semantic accents
picked to stand out from a deliberately muted ramp.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| B0 | `#26232e` | sfondo app, header, sidebar | app background, header, sidebar |
| B1 | `#3b3749` | superficie delle card | card surface |
| B2 | `#524d68` | bordi, divider, disabilitati | borders, dividers, disabled |
| B3 | `#6b6488` | accent, slider, selezioni | accent, sliders, selections |
| B4 | `#8781a5` | colore primario, icone attive | primary colour, active icons |
| B5 | `#a5a0c0` | hover, stati attivi | hover, active states |
| B6 | `#c9c5db` | testo secondario | secondary text |
| B7 | `#f0eef7` | testo primario | primary text |

**Semantici / Semantic** — `#cf6470` errore·error · `#ddab55` avviso·warning ·
`#7fb98a` successo·success · `#ecc76d` luci accese·lights on
...
```
Brughiera/
├── brughiera.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── brughiera-bg.svg                  # sorgente vettoriale / vector source
    ├── brughiera-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── brughiera-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `brughiera` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/brughiera/brughiera-bg-2560x1440.png
         homeassistant/www/brughiera/brughiera-bg-1170x2532.png

altre  ▸  config/www/brughiera/brughiera-bg-2560x1440.png
          config/www/brughiera/brughiera-bg-1170x2532.png
```

> La sottocartella `brughiera` non è facoltativa: il tema punta a
> `/local/brughiera/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `brughiera/` dalla
> riga `lovelace-background` in `brughiera.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `brughiera` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `brughiera`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `brughiera.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #211f28 0%, #26232e 45%, #353141 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/brughiera.yaml
altre  ▸  config/themes/brughiera.yaml
```
...
```
themes/brughiera.yaml
```

File editor crea la cartella `themes` da solo. A quel punto incolla dentro il
contenuto di `brughiera.yaml` e salva con l'icona del dischetto.
...
`brughiera.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Brughiera**.

> Se **Brughiera** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/brughiera.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `brughiera.yaml` sostituendo il nome
del file con `brughiera-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/brughiera/brughiera-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/brughiera/` dentro la cartella di configurazione.
...
create a `brughiera` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/brughiera/brughiera-bg-2560x1440.png
          homeassistant/www/brughiera/brughiera-bg-1170x2532.png

others ▸  config/www/brughiera/brughiera-bg-2560x1440.png
          config/www/brughiera/brughiera-bg-1170x2532.png
```

> The `brughiera` subfolder is not optional: the theme points at `/local/brughiera/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `brughiera/` from the
> `lovelace-background` line in `brughiera.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`brughiera` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `brughiera`, then drag the PNGs in.
...
The theme works without the image. Open `brughiera.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/brughiera.yaml
others ▸  config/themes/brughiera.yaml
```
...
`brughiera.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `brughiera.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Brughiera**.

> If **Brughiera** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/brughiera.yaml` and not in the config root;
...
edit the `lovelace-background` line in `brughiera.yaml` and swap the filename for
`brughiera-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/brughiera/brughiera-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/brughiera/` inside the configuration folder.
