# Neon — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro ad alta saturazione per Home Assistant, derivato dalla palette **Neon** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal viola quasi nero al rosa fluo, piu quattro accenti
semantici altrettanto accesi, per non perdersi in mezzo alla rampa.

A high-saturation dark theme for Home Assistant, derived from the **Neon**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from near-black violet to fluorescent pink, plus four equally
vivid semantic accents so they do not get lost against the ramp.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| N0 | `#0d0221` | sfondo app, header, sidebar | app background, header, sidebar |
| N1 | `#33095e` | superficie delle card | card surface |
| N2 | `#6a0dad` | bordi, divider, disabilitati | borders, dividers, disabled |
| N3 | `#a319c4` | accent, slider, selezioni | accent, sliders, selections |
| N4 | `#e01e84` | colore primario, icone attive | primary colour, active icons |
| N5 | `#ff2079` | hover, stati attivi | hover, active states |
| N6 | `#ff7ab6` | testo secondario | secondary text |
| N7 | `#ffd3e8` | testo primario | primary text |

**Semantici / Semantic** — `#ff3131` errore·error · `#ffcc00` avviso·warning ·
`#00f5a0` successo·success · `#fff275` luci accese·lights on
...
```
Neon/
├── neon.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── neon-bg.svg                  # sorgente vettoriale / vector source
    ├── neon-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── neon-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `neon` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/neon/neon-bg-2560x1440.png
         homeassistant/www/neon/neon-bg-1170x2532.png

altre  ▸  config/www/neon/neon-bg-2560x1440.png
          config/www/neon/neon-bg-1170x2532.png
```

> La sottocartella `neon` non è facoltativa: il tema punta a
> `/local/neon/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `neon/` dalla
> riga `lovelace-background` in `neon.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `neon` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `neon`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `neon.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #0b021d 0%, #0d0221 45%, #28074c 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/neon.yaml
altre  ▸  config/themes/neon.yaml
```
...
contenuto di `neon.yaml` e salva con l'icona del dischetto.
...
`neon.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Neon**.

> Se **Neon** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/neon.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `neon.yaml` sostituendo il nome
del file con `neon-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/neon/neon-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/neon/` dentro la cartella di configurazione.
...
create a `neon` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/neon/neon-bg-2560x1440.png
          homeassistant/www/neon/neon-bg-1170x2532.png

others ▸  config/www/neon/neon-bg-2560x1440.png
          config/www/neon/neon-bg-1170x2532.png
```

> The `neon` subfolder is not optional: the theme points at `/local/neon/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `neon/` from the
> `lovelace-background` line in `neon.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`neon` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `neon`, then drag the PNGs in.
...
The theme works without the image. Open `neon.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/neon.yaml
others ▸  config/themes/neon.yaml
```
...
`neon.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `neon.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Neon**.

> If **Neon** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/neon.yaml` and not in the config root;
...
edit the `lovelace-background` line in `neon.yaml` and swap the filename for
`neon-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/neon/neon-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/neon/` inside the configuration folder.
