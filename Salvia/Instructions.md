# Salvia — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro verde-salvia per Home Assistant, derivato dalla palette **Salvia** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal verde-antracite al bianco erbaceo, più tre accenti semantici caldi
e desaturati per allarmi, avvisi e conferme.

A sage-green dark theme for Home Assistant, derived from the **Salvia** palette
used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from deep green-charcoal to herbal white, plus three warm, desaturated
semantic accents for alarms, warnings and confirmations.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| S0 | `#2f3e46` | sfondo app, header, sidebar | app background, header, sidebar |
| S1 | `#435c54` | superficie delle card | card surface |
| S2 | `#52796f` | bordi, divider, disabilitati | borders, dividers, disabled |
| S3 | `#6f9482` | accent, slider, selezioni | accent, sliders, selections |
| S4 | `#84a98c` | colore primario, icone attive | primary colour, active icons |
| S5 | `#a5c3a0` | hover, stati attivi | hover, active states |
| S6 | `#cad2c5` | testo secondario | secondary text |
| S7 | `#eef1ec` | testo primario | primary text |

**Semantici / Semantic** — `#c1666b` errore·error · `#d9ae5f` avviso·warning ·
`#7fb069` successo·success · `#e9c46a` luci accese·lights on
...
```
Salvia/
├── salvia.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── salvia-bg.svg                  # sorgente vettoriale / vector source
    ├── salvia-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── salvia-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `salvia` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/salvia/salvia-bg-2560x1440.png
         homeassistant/www/salvia/salvia-bg-1170x2532.png

altre  ▸  config/www/salvia/salvia-bg-2560x1440.png
          config/www/salvia/salvia-bg-1170x2532.png
```

> La sottocartella `salvia` non è facoltativa: il tema punta a
> `/local/salvia/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `salvia/` dalla
> riga `lovelace-background` in `salvia.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `salvia` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `salvia`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `salvia.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #29363d 0%, #2f3e46 45%, #3c5049 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/salvia.yaml
altre  ▸  config/themes/salvia.yaml
```
...
contenuto di `salvia.yaml` e salva con l'icona del dischetto.
...
`salvia.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Salvia**.

> Se **Salvia** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/salvia.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `salvia.yaml` sostituendo il nome
del file con `salvia-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/salvia/salvia-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/salvia/` dentro la cartella di configurazione.
...
create a `salvia` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/salvia/salvia-bg-2560x1440.png
          homeassistant/www/salvia/salvia-bg-1170x2532.png

others ▸  config/www/salvia/salvia-bg-2560x1440.png
          config/www/salvia/salvia-bg-1170x2532.png
```

> The `salvia` subfolder is not optional: the theme points at `/local/salvia/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `salvia/` from the
> `lovelace-background` line in `salvia.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`salvia` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `salvia`, then drag the PNGs in.
...
The theme works without the image. Open `salvia.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/salvia.yaml
others ▸  config/themes/salvia.yaml
```
...
`salvia.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `salvia.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Salvia**.

> If **Salvia** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/salvia.yaml` and not in the config root;
...
edit the `lovelace-background` line in `salvia.yaml` and swap the filename for
`salvia-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/salvia/salvia-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/salvia/` inside the configuration folder.
