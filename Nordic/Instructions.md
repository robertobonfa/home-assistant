# Nordic — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro freddo per Home Assistant, derivato dalla palette **Nordic** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal Polar Night alla Snow Storm, più tre accenti semantici della
palette [Nord](https://www.nordtheme.com/) per allarmi, avvisi e conferme.

A cold dark theme for Home Assistant, derived from the **Nordic** palette used on
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from Polar Night to Snow Storm, plus three semantic accents from the
[Nord](https://www.nordtheme.com/) palette for alarms, warnings and confirmations.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| N0 | `#2e3440` | sfondo app, header, sidebar | app background, header, sidebar |
| N1 | `#3b4252` | superficie delle card | card surface |
| N2 | `#4c566a` | bordi, divider, disabilitati | borders, dividers, disabled |
| N3 | `#5e81ac` | accent, slider, selezioni | accent, sliders, selections |
| N4 | `#81a1c1` | colore primario, icone attive | primary colour, active icons |
| N5 | `#94bad3` | hover, stati attivi | hover, active states |
| N6 | `#b8cede` | testo secondario | secondary text |
| N7 | `#eceff4` | testo primario | primary text |

**Semantici / Semantic** — `#bf616a` errore·error · `#ebcb8b` avviso·warning ·
`#a3be8c` successo·success · `#b48ead` luci accese·lights on
...
```
Nordic/
├── nordic.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── nordic-bg.svg                  # sorgente vettoriale / vector source
    ├── nordic-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── nordic-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `nordic` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/nordic/nordic-bg-2560x1440.png
         homeassistant/www/nordic/nordic-bg-1170x2532.png

altre  ▸  config/www/nordic/nordic-bg-2560x1440.png
          config/www/nordic/nordic-bg-1170x2532.png
```

> La sottocartella `nordic` non è facoltativa: il tema punta a
> `/local/nordic/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `nordic/` dalla
> riga `lovelace-background` in `nordic.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `nordic` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `nordic`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `nordic.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #272c36 0%, #2e3440 45%, #39404e 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/nordic.yaml
altre  ▸  config/themes/nordic.yaml
```
...
contenuto di `nordic.yaml` e salva con l'icona del dischetto.
...
`nordic.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Nordic**.

> Se **Nordic** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/nordic.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `nordic.yaml` sostituendo il nome
del file con `nordic-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/nordic/nordic-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/nordic/` dentro la cartella di configurazione.
...
create a `nordic` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/nordic/nordic-bg-2560x1440.png
          homeassistant/www/nordic/nordic-bg-1170x2532.png

others ▸  config/www/nordic/nordic-bg-2560x1440.png
          config/www/nordic/nordic-bg-1170x2532.png
```

> The `nordic` subfolder is not optional: the theme points at `/local/nordic/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `nordic/` from the
> `lovelace-background` line in `nordic.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`nordic` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `nordic`, then drag the PNGs in.
...
The theme works without the image. Open `nordic.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/nordic.yaml
others ▸  config/themes/nordic.yaml
```
...
`nordic.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `nordic.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Nordic**.

> If **Nordic** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/nordic.yaml` and not in the config root;
...
edit the `lovelace-background` line in `nordic.yaml` and swap the filename for
`nordic-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/nordic/nordic-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/nordic/` inside the configuration folder.
...
Palette Nord di Arctic Ice Studio e Sven Greb, MIT.
*Nord palette by Arctic Ice Studio and Sven Greb, MIT.*
