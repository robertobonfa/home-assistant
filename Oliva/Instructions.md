# Oliva — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro verde oliva per Home Assistant, derivato dalla palette **Oliva** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dall'oliva quasi nera al bianco lino, piu quattro accenti
semantici scelti per staccare dai verdi-gialli della rampa.

An olive-green dark theme for Home Assistant, derived from the **Oliva**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from near-black olive to linen white, plus four semantic accents
picked to stand apart from the ramp's yellow-greens.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| O0 | `#20240f` | sfondo app, header, sidebar | app background, header, sidebar |
| O1 | `#363d1e` | superficie delle card | card surface |
| O2 | `#4f5730` | bordi, divider, disabilitati | borders, dividers, disabled |
| O3 | `#687244` | accent, slider, selezioni | accent, sliders, selections |
| O4 | `#83905e` | colore primario, icone attive | primary colour, active icons |
| O5 | `#a4ad80` | hover, stati attivi | hover, active states |
| O6 | `#c8cea8` | testo secondario | secondary text |
| O7 | `#f1f2e1` | testo primario | primary text |

**Semantici / Semantic** — `#cf5b4e` errore·error · `#e8a33d` avviso·warning ·
`#7ec46a` successo·success · `#f2d65e` luci accese·lights on
...
```
Oliva/
├── oliva.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── oliva-bg.svg                  # sorgente vettoriale / vector source
    ├── oliva-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── oliva-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `oliva` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/oliva/oliva-bg-2560x1440.png
         homeassistant/www/oliva/oliva-bg-1170x2532.png

altre  ▸  config/www/oliva/oliva-bg-2560x1440.png
          config/www/oliva/oliva-bg-1170x2532.png
```

> La sottocartella `oliva` non è facoltativa: il tema punta a
> `/local/oliva/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `oliva/` dalla
> riga `lovelace-background` in `oliva.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `oliva` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `oliva`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `oliva.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #1c200d 0%, #20240f 45%, #2f361a 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/oliva.yaml
altre  ▸  config/themes/oliva.yaml
```
...
contenuto di `oliva.yaml` e salva con l'icona del dischetto.
...
`oliva.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Oliva**.

> Se **Oliva** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/oliva.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `oliva.yaml` sostituendo il nome
del file con `oliva-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/oliva/oliva-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/oliva/` dentro la cartella di configurazione.
...
create a `oliva` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/oliva/oliva-bg-2560x1440.png
          homeassistant/www/oliva/oliva-bg-1170x2532.png

others ▸  config/www/oliva/oliva-bg-2560x1440.png
          config/www/oliva/oliva-bg-1170x2532.png
```

> The `oliva` subfolder is not optional: the theme points at `/local/oliva/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `oliva/` from the
> `lovelace-background` line in `oliva.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`oliva` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `oliva`, then drag the PNGs in.
...
The theme works without the image. Open `oliva.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/oliva.yaml
others ▸  config/themes/oliva.yaml
```
...
`oliva.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `oliva.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Oliva**.

> If **Oliva** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/oliva.yaml` and not in the config root;
...
edit the `lovelace-background` line in `oliva.yaml` and swap the filename for
`oliva-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/oliva/oliva-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/oliva/` inside the configuration folder.
