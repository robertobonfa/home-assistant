# Petrolio — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro verde-petrolio per Home Assistant, derivato dalla palette **Petrolio** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal petrolio profondo al bianco vapore, piu quattro accenti
semantici scelti per restare leggibili sui verdazzurri smorzati.

A petrol-teal dark theme for Home Assistant, derived from the **Petrolio**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from deep petrol to steam white, plus four semantic accents
picked to stay legible against muted teals.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| P0 | `#12262c` | sfondo app, header, sidebar | app background, header, sidebar |
| P1 | `#1d3c44` | superficie delle card | card surface |
| P2 | `#2c545c` | bordi, divider, disabilitati | borders, dividers, disabled |
| P3 | `#417076` | accent, slider, selezioni | accent, sliders, selections |
| P4 | `#5d8d90` | colore primario, icone attive | primary colour, active icons |
| P5 | `#83aeae` | hover, stati attivi | hover, active states |
| P6 | `#b4d0cd` | testo secondario | secondary text |
| P7 | `#e9f3f0` | testo primario | primary text |

**Semantici / Semantic** — `#d9646a` errore·error · `#e0ad55` avviso·warning ·
`#7fbf7a` successo·success · `#edc96a` luci accese·lights on
...
```
Petrolio/
├── petrolio.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── petrolio-bg.svg                  # sorgente vettoriale / vector source
    ├── petrolio-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── petrolio-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `petrolio` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/petrolio/petrolio-bg-2560x1440.png
         homeassistant/www/petrolio/petrolio-bg-1170x2532.png

altre  ▸  config/www/petrolio/petrolio-bg-2560x1440.png
          config/www/petrolio/petrolio-bg-1170x2532.png
```

> La sottocartella `petrolio` non è facoltativa: il tema punta a
> `/local/petrolio/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `petrolio/` dalla
> riga `lovelace-background` in `petrolio.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `petrolio` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `petrolio`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `petrolio.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #102127 0%, #12262c 45%, #1a353d 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/petrolio.yaml
altre  ▸  config/themes/petrolio.yaml
```
...
```
themes/petrolio.yaml
```

File editor crea la cartella `themes` da solo. A quel punto incolla dentro il
contenuto di `petrolio.yaml` e salva con l'icona del dischetto.
...
`petrolio.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Petrolio**.

> Se **Petrolio** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/petrolio.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `petrolio.yaml` sostituendo il nome
del file con `petrolio-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/petrolio/petrolio-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/petrolio/` dentro la cartella di configurazione.
...
create a `petrolio` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/petrolio/petrolio-bg-2560x1440.png
          homeassistant/www/petrolio/petrolio-bg-1170x2532.png

others ▸  config/www/petrolio/petrolio-bg-2560x1440.png
          config/www/petrolio/petrolio-bg-1170x2532.png
```

> The `petrolio` subfolder is not optional: the theme points at `/local/petrolio/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `petrolio/` from the
> `lovelace-background` line in `petrolio.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`petrolio` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `petrolio`, then drag the PNGs in.
...
The theme works without the image. Open `petrolio.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/petrolio.yaml
others ▸  config/themes/petrolio.yaml
```
...
`petrolio.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `petrolio.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Petrolio**.

> If **Petrolio** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/petrolio.yaml` and not in the config root;
...
edit the `lovelace-background` line in `petrolio.yaml` and swap the filename for
`petrolio-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/petrolio/petrolio-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/petrolio/` inside the configuration folder.
