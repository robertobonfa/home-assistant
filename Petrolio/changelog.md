...
- Gli interruttori accesi usavano `#83aeae`, lo stesso colore dello stato
  ...
  `state-switch-active-color` passa a `#edc96a`.
...
- Tema `petrolio.yaml` per Home Assistant, 185 variabili, interamente dichiarato
  sotto `modes: dark:`.
- Mappatura completa della rampa Petrolio a 8 stop (`#12262c` → `#e9f3f0`) presa
  dalla palette omonima di `wallpapers.robertobonfa.com` (`js/app.js:145`).
- Accenti semantici: `#d9646a` errore, `#e0ad55` avviso, `#7fbf7a` successo,
  `#edc96a` per il dominio `light` attivo.
...
- Sfondo minimale: sorgente `backgrounds/petrolio-bg.svg` più PNG 2560×1440
