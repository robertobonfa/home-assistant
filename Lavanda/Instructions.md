# Lavanda — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro viola-lavanda per Home Assistant, derivato dalla palette **Lavanda** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal notturno profondo al rosa cipria, piu quattro accenti
semantici scelti per restare leggibili su fondo viola.

A lavender-purple dark theme for Home Assistant, derived from the **Lavanda**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from deep night violet to powder pink, plus four semantic
accents picked to stay legible against purple.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| L0 | `#231942` | sfondo app, header, sidebar | app background, header, sidebar |
| L1 | `#3d2c66` | superficie delle card | card surface |
| L2 | `#5e548e` | bordi, divider, disabilitati | borders, dividers, disabled |
| L3 | `#7d6ba0` | accent, slider, selezioni | accent, sliders, selections |
| L4 | `#9f86c0` | colore primario, icone attive | primary colour, active icons |
| L5 | `#be95c4` | hover, stati attivi | hover, active states |
| L6 | `#e0b1cb` | testo secondario | secondary text |
| L7 | `#f7e8f0` | testo primario | primary text |

**Semantici / Semantic** — `#e04f5f` errore·error · `#e8b44c` avviso·warning ·
`#7fc09a` successo·success · `#f3cf6b` luci accese·lights on
...
```
Lavanda/
├── lavanda.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── lavanda-bg.svg                  # sorgente vettoriale / vector source
    ├── lavanda-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── lavanda-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `lavanda` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/lavanda/lavanda-bg-2560x1440.png
         homeassistant/www/lavanda/lavanda-bg-1170x2532.png

altre  ▸  config/www/lavanda/lavanda-bg-2560x1440.png
          config/www/lavanda/lavanda-bg-1170x2532.png
```

> La sottocartella `lavanda` non è facoltativa: il tema punta a
> `/local/lavanda/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `lavanda/` dalla
> riga `lovelace-background` in `lavanda.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `lavanda` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `lavanda`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `lavanda.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #1f163a 0%, #231942 45%, #35265b 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/lavanda.yaml
altre  ▸  config/themes/lavanda.yaml
```
...
contenuto di `lavanda.yaml` e salva con l'icona del dischetto.
...
`lavanda.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Lavanda**.

> Se **Lavanda** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/lavanda.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `lavanda.yaml` sostituendo il nome
del file con `lavanda-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/lavanda/lavanda-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/lavanda/` dentro la cartella di configurazione.
...
create a `lavanda` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/lavanda/lavanda-bg-2560x1440.png
          homeassistant/www/lavanda/lavanda-bg-1170x2532.png

others ▸  config/www/lavanda/lavanda-bg-2560x1440.png
          config/www/lavanda/lavanda-bg-1170x2532.png
```

> The `lavanda` subfolder is not optional: the theme points at `/local/lavanda/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `lavanda/` from the
> `lovelace-background` line in `lavanda.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`lavanda` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `lavanda`, then drag the PNGs in.
...
The theme works without the image. Open `lavanda.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/lavanda.yaml
others ▸  config/themes/lavanda.yaml
```
...
`lavanda.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `lavanda.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Lavanda**.

> If **Lavanda** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/lavanda.yaml` and not in the config root;
...
edit the `lovelace-background` line in `lavanda.yaml` and swap the filename for
`lavanda-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/lavanda/lavanda-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/lavanda/` inside the configuration folder.
