...
- Gli interruttori accesi usavano `#be95c4`, lo stesso colore dello stato
  ...
  `state-switch-active-color` passa a `#f3cf6b`.
...
- Tema `lavanda.yaml` per Home Assistant, 185 variabili, interamente dichiarato
  sotto `modes: dark:`.
- Mappatura completa della rampa Lavanda a 8 stop (`#231942` → `#f7e8f0`) presa
  dalla palette omonima di `wallpapers.robertobonfa.com` (`js/app.js:131`).
- Accenti semantici: `#e04f5f` errore, `#e8b44c` avviso, `#7fc09a` successo,
  `#f3cf6b` per il dominio `light` attivo.
...
- Sfondo minimale: sorgente `backgrounds/lavanda-bg.svg` più PNG 2560×1440
