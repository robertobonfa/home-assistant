# Notte — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro blu notte per Home Assistant, derivato dalla palette **Notte** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal blu quasi nero all'azzurro polvere, piu quattro accenti
semantici scelti per restare leggibili su fondo blu profondo.

A midnight-blue dark theme for Home Assistant, derived from the **Notte**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from near-black navy to dusty light blue, plus four semantic
accents picked to stay legible against deep blue.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| N0 | `#0b0d21` | sfondo app, header, sidebar | app background, header, sidebar |
| N1 | `#151a3d` | superficie delle card | card surface |
| N2 | `#232a63` | bordi, divider, disabilitati | borders, dividers, disabled |
| N3 | `#3a4494` | accent, slider, selezioni | accent, sliders, selections |
| N4 | `#5a6ac9` | colore primario, icone attive | primary colour, active icons |
| N5 | `#8b9ae8` | hover, stati attivi | hover, active states |
| N6 | `#c3ccf7` | testo secondario | secondary text |
| N7 | `#f0f3ff` | testo primario | primary text |

**Semantici / Semantic** — `#e5556a` errore·error · `#edb54b` avviso·warning ·
`#5cc99a` successo·success · `#f2d16b` luci accese·lights on
...
```
Notte/
├── notte.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── notte-bg.svg                  # sorgente vettoriale / vector source
    ├── notte-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── notte-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `notte` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/notte/notte-bg-2560x1440.png
         homeassistant/www/notte/notte-bg-1170x2532.png

altre  ▸  config/www/notte/notte-bg-2560x1440.png
          config/www/notte/notte-bg-1170x2532.png
```

> La sottocartella `notte` non è facoltativa: il tema punta a
> `/local/notte/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `notte/` dalla
> riga `lovelace-background` in `notte.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `notte` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `notte`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `notte.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #0a0b1d 0%, #0b0d21 45%, #121635 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/notte.yaml
altre  ▸  config/themes/notte.yaml
```
...
contenuto di `notte.yaml` e salva con l'icona del dischetto.
...
`notte.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Notte**.

> Se **Notte** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/notte.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `notte.yaml` sostituendo il nome
del file con `notte-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/notte/notte-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/notte/` dentro la cartella di configurazione.
...
create a `notte` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/notte/notte-bg-2560x1440.png
          homeassistant/www/notte/notte-bg-1170x2532.png

others ▸  config/www/notte/notte-bg-2560x1440.png
          config/www/notte/notte-bg-1170x2532.png
```

> The `notte` subfolder is not optional: the theme points at `/local/notte/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `notte/` from the
> `lovelace-background` line in `notte.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`notte` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `notte`, then drag the PNGs in.
...
The theme works without the image. Open `notte.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/notte.yaml
others ▸  config/themes/notte.yaml
```
...
`notte.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `notte.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Notte**.

> If **Notte** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/notte.yaml` and not in the config root;
...
edit the `lovelace-background` line in `notte.yaml` and swap the filename for
`notte-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/notte/notte-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/notte/` inside the configuration folder.
