...
- Gli interruttori accesi usavano `#8b9ae8`, lo stesso colore dello stato
  ...
  `state-switch-active-color` passa a `#f2d16b`.
...
- Tema `notte.yaml` per Home Assistant, 185 variabili, interamente dichiarato
  sotto `modes: dark:`.
- Mappatura completa della rampa Notte a 8 stop (`#0b0d21` → `#f0f3ff`) presa
  dalla palette omonima di `wallpapers.robertobonfa.com` (`js/app.js:140`).
- Accenti semantici: `#e5556a` errore, `#edb54b` avviso, `#5cc99a` successo,
  `#f2d16b` per il dominio `light` attivo.
...
- Sfondo minimale: sorgente `backgrounds/notte-bg.svg` più PNG 2560×1440
