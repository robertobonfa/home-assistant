# Argilla — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro terroso per Home Assistant, derivato dalla palette **Argilla** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dalla terra bruciata alla sabbia chiara, piu quattro accenti
semantici scelti per non confondersi con i bruni della rampa.

An earthy dark theme for Home Assistant, derived from the **Argilla**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from burnt earth to pale sand, plus four semantic accents
picked not to blend into the palette's own browns.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| A0 | `#2b1a16` | sfondo app, header, sidebar | app background, header, sidebar |
| A1 | `#4c2e25` | superficie delle card | card surface |
| A2 | `#6d4536` | bordi, divider, disabilitati | borders, dividers, disabled |
| A3 | `#8d5c48` | accent, slider, selezioni | accent, sliders, selections |
| A4 | `#a97862` | colore primario, icone attive | primary colour, active icons |
| A5 | `#c69a82` | hover, stati attivi | hover, active states |
| A6 | `#ddc1a8` | testo secondario | secondary text |
| A7 | `#f6ecdf` | testo primario | primary text |

**Semantici / Semantic** — `#d4453f` errore·error · `#e8b13c` avviso·warning ·
`#7fa860` successo·success · `#f0c46a` luci accese·lights on
...
```
Argilla/
├── argilla.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── argilla-bg.svg                  # sorgente vettoriale / vector source
    ├── argilla-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── argilla-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `argilla` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/argilla/argilla-bg-2560x1440.png
         homeassistant/www/argilla/argilla-bg-1170x2532.png

altre  ▸  config/www/argilla/argilla-bg-2560x1440.png
          config/www/argilla/argilla-bg-1170x2532.png
```

> La sottocartella `argilla` non è facoltativa: il tema punta a
> `/local/argilla/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `argilla/` dalla
> riga `lovelace-background` in `argilla.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `argilla` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `argilla`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `argilla.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #261713 0%, #2b1a16 45%, #422820 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/argilla.yaml
altre  ▸  config/themes/argilla.yaml
```
...
contenuto di `argilla.yaml` e salva con l'icona del dischetto.
...
`argilla.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Argilla**.

> Se **Argilla** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/argilla.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `argilla.yaml` sostituendo il nome
del file con `argilla-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/argilla/argilla-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/argilla/` dentro la cartella di configurazione.
...
create a `argilla` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/argilla/argilla-bg-2560x1440.png
          homeassistant/www/argilla/argilla-bg-1170x2532.png

others ▸  config/www/argilla/argilla-bg-2560x1440.png
          config/www/argilla/argilla-bg-1170x2532.png
```

> The `argilla` subfolder is not optional: the theme points at `/local/argilla/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `argilla/` from the
> `lovelace-background` line in `argilla.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`argilla` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `argilla`, then drag the PNGs in.
...
The theme works without the image. Open `argilla.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/argilla.yaml
others ▸  config/themes/argilla.yaml
```
...
`argilla.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `argilla.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Argilla**.

> If **Argilla** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/argilla.yaml` and not in the config root;
...
edit the `lovelace-background` line in `argilla.yaml` and swap the filename for
`argilla-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/argilla/argilla-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/argilla/` inside the configuration folder.
