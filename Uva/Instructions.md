# Uva — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro viola intenso per Home Assistant, derivato dalla palette **Uva** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal melanzana al lillà chiarissimo, piu quattro accenti
semantici scelti per staccare da un viola molto saturo.

A deep-purple dark theme for Home Assistant, derived from the **Uva**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from aubergine to the palest lilac, plus four semantic
accents picked to stand out against a highly saturated purple.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| U0 | `#1e0a24` | sfondo app, header, sidebar | app background, header, sidebar |
| U1 | `#3c1450` | superficie delle card | card surface |
| U2 | `#5e2382` | bordi, divider, disabilitati | borders, dividers, disabled |
| U3 | `#8236b5` | accent, slider, selezioni | accent, sliders, selections |
| U4 | `#a855d8` | colore primario, icone attive | primary colour, active icons |
| U5 | `#c77dff` | hover, stati attivi | hover, active states |
| U6 | `#e0aaff` | testo secondario | secondary text |
| U7 | `#f6ebff` | testo primario | primary text |

**Semantici / Semantic** — `#e63950` errore·error · `#f5b942` avviso·warning ·
`#5ecfa0` successo·success · `#ffd166` luci accese·lights on
...
```
Uva/
├── uva.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── uva-bg.svg                  # sorgente vettoriale / vector source
    ├── uva-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── uva-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `uva` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/uva/uva-bg-2560x1440.png
         homeassistant/www/uva/uva-bg-1170x2532.png

altre  ▸  config/www/uva/uva-bg-2560x1440.png
          config/www/uva/uva-bg-1170x2532.png
```

> La sottocartella `uva` non è facoltativa: il tema punta a
> `/local/uva/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `uva/` dalla
> riga `lovelace-background` in `uva.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `uva` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `uva`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `uva.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #1a0920 0%, #1e0a24 45%, #331143 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/uva.yaml
altre  ▸  config/themes/uva.yaml
```
...
contenuto di `uva.yaml` e salva con l'icona del dischetto.
...
`uva.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Uva**.

> Se **Uva** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/uva.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `uva.yaml` sostituendo il nome
del file con `uva-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/uva/uva-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/uva/` dentro la cartella di configurazione.
...
create a `uva` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/uva/uva-bg-2560x1440.png
          homeassistant/www/uva/uva-bg-1170x2532.png

others ▸  config/www/uva/uva-bg-2560x1440.png
          config/www/uva/uva-bg-1170x2532.png
```

> The `uva` subfolder is not optional: the theme points at `/local/uva/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `uva/` from the
> `lovelace-background` line in `uva.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`uva` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `uva`, then drag the PNGs in.
...
The theme works without the image. Open `uva.yaml` and replace the
...
`uva.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `uva.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Uva**.

> If **Uva** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/uva.yaml` and not in the config root;
...
edit the `lovelace-background` line in `uva.yaml` and swap the filename for
`uva-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/uva/uva-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/uva/` inside the configuration folder.
