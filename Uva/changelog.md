...
- Gli interruttori accesi usavano `#c77dff`, lo stesso colore dello stato
  ...
  `state-switch-active-color` passa a `#ffd166`.
...
- Tema `uva.yaml` per Home Assistant, 185 variabili, interamente dichiarato
  sotto `modes: dark:`.
- Mappatura completa della rampa Uva a 8 stop (`#1e0a24` → `#f6ebff`) presa
  dalla palette omonima di `wallpapers.robertobonfa.com` (`js/app.js:142`).
- Accenti semantici: `#e63950` errore, `#f5b942` avviso, `#5ecfa0` successo,
...
- Sfondo minimale: sorgente `backgrounds/uva-bg.svg` più PNG 2560×1440
