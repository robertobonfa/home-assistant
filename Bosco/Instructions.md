# Bosco — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro verde bosco per Home Assistant, derivato dalla palette **Bosco** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal sottobosco quasi nero al verde brina, piu quattro accenti
semantici scelti per staccare da una rampa interamente verde.

A forest-green dark theme for Home Assistant, derived from the **Bosco**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from near-black undergrowth to frosted green, plus four
semantic accents picked to stand apart from an all-green ramp.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| B0 | `#081c15` | sfondo app, header, sidebar | app background, header, sidebar |
| B1 | `#1b4332` | superficie delle card | card surface |
| B2 | `#2d6a4f` | bordi, divider, disabilitati | borders, dividers, disabled |
| B3 | `#40916c` | accent, slider, selezioni | accent, sliders, selections |
| B4 | `#52b788` | colore primario, icone attive | primary colour, active icons |
| B5 | `#74c69d` | hover, stati attivi | hover, active states |
| B6 | `#b7e4c7` | testo secondario | secondary text |
| B7 | `#e8f5ee` | testo primario | primary text |

**Semantici / Semantic** — `#d9534f` errore·error · `#e6a23c` avviso·warning ·
`#a3d977` successo·success · `#f2c14e` luci accese·lights on
...
```
Bosco/
├── bosco.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── bosco-bg.svg                  # sorgente vettoriale / vector source
    ├── bosco-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── bosco-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `bosco` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/bosco/bosco-bg-2560x1440.png
         homeassistant/www/bosco/bosco-bg-1170x2532.png

altre  ▸  config/www/bosco/bosco-bg-2560x1440.png
          config/www/bosco/bosco-bg-1170x2532.png
```

> La sottocartella `bosco` non è facoltativa: il tema punta a
> `/local/bosco/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `bosco/` dalla
> riga `lovelace-background` in `bosco.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `bosco` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `bosco`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `bosco.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #071912 0%, #081c15 45%, #153729 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/bosco.yaml
altre  ▸  config/themes/bosco.yaml
```
...
contenuto di `bosco.yaml` e salva con l'icona del dischetto.
...
`bosco.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Bosco**.

> Se **Bosco** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/bosco.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `bosco.yaml` sostituendo il nome
del file con `bosco-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/bosco/bosco-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/bosco/` dentro la cartella di configurazione.
...
create a `bosco` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/bosco/bosco-bg-2560x1440.png
          homeassistant/www/bosco/bosco-bg-1170x2532.png

others ▸  config/www/bosco/bosco-bg-2560x1440.png
          config/www/bosco/bosco-bg-1170x2532.png
```

> The `bosco` subfolder is not optional: the theme points at `/local/bosco/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `bosco/` from the
> `lovelace-background` line in `bosco.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`bosco` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `bosco`, then drag the PNGs in.
...
The theme works without the image. Open `bosco.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/bosco.yaml
others ▸  config/themes/bosco.yaml
```
...
`bosco.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `bosco.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Bosco**.

> If **Bosco** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/bosco.yaml` and not in the config root;
...
edit the `lovelace-background` line in `bosco.yaml` and swap the filename for
`bosco-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/bosco/bosco-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/bosco/` inside the configuration folder.
