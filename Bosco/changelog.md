...
- Gli interruttori accesi usavano `#74c69d`, lo stesso colore dello stato
  ...
  `state-switch-active-color` passa a `#f2c14e`.
...
- Tema `bosco.yaml` per Home Assistant, 185 variabili, interamente dichiarato
  sotto `modes: dark:`.
- Mappatura completa della rampa Bosco a 8 stop (`#081c15` → `#e8f5ee`) presa
  dalla palette omonima di `wallpapers.robertobonfa.com` (`js/app.js:133`).
- Accenti semantici: `#d9534f` errore, `#e6a23c` avviso, `#a3d977` successo,
  `#f2c14e` per il dominio `light` attivo.
...
- Sfondo minimale: sorgente `backgrounds/bosco-bg.svg` più PNG 2560×1440
