# Terracotta — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro caldo per Home Assistant, derivato dalla palette **Terracotta** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dall'ardesia al bianco sabbia passando per il cotto, piu quattro
accenti semantici scelti per non confondersi con i caldi della rampa.

A warm dark theme for Home Assistant, derived from the **Terracotta**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from slate through fired clay to sand white, plus four semantic
accents picked not to collide with the palette's own warm tones.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| T0 | `#3d405b` | sfondo app, header, sidebar | app background, header, sidebar |
| T1 | `#6d4a53` | superficie delle card | card surface |
| T2 | `#9c544d` | bordi, divider, disabilitati | borders, dividers, disabled |
| T3 | `#c96a52` | accent, slider, selezioni | accent, sliders, selections |
| T4 | `#e07a5f` | colore primario, icone attive | primary colour, active icons |
| T5 | `#eda47f` | hover, stati attivi | hover, active states |
| T6 | `#f2cc8f` | testo secondario | secondary text |
| T7 | `#f9f1de` | testo primario | primary text |

**Semantici / Semantic** — `#d62839` errore·error · `#f0a202` avviso·warning ·
`#6a994e` successo·success · `#ffd166` luci accese·lights on
...
```
Terracotta/
├── terracotta.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── terracotta-bg.svg                  # sorgente vettoriale / vector source
    ├── terracotta-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── terracotta-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `terracotta` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/terracotta/terracotta-bg-2560x1440.png
         homeassistant/www/terracotta/terracotta-bg-1170x2532.png

altre  ▸  config/www/terracotta/terracotta-bg-2560x1440.png
          config/www/terracotta/terracotta-bg-1170x2532.png
```

> La sottocartella `terracotta` non è facoltativa: il tema punta a
> `/local/terracotta/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `terracotta/` dalla
> riga `lovelace-background` in `terracotta.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `terracotta` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `terracotta`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `terracotta.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #363850 0%, #3d405b 45%, #5f4755 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/terracotta.yaml
altre  ▸  config/themes/terracotta.yaml
```
...
```
themes/terracotta.yaml
```

File editor crea la cartella `themes` da solo. A quel punto incolla dentro il
contenuto di `terracotta.yaml` e salva con l'icona del dischetto.
...
`terracotta.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Terracotta**.

> Se **Terracotta** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/terracotta.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `terracotta.yaml` sostituendo il nome
del file con `terracotta-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/terracotta/terracotta-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/terracotta/` dentro la cartella di configurazione.
...
create a `terracotta` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/terracotta/terracotta-bg-2560x1440.png
          homeassistant/www/terracotta/terracotta-bg-1170x2532.png

others ▸  config/www/terracotta/terracotta-bg-2560x1440.png
          config/www/terracotta/terracotta-bg-1170x2532.png
```

> The `terracotta` subfolder is not optional: the theme points at `/local/terracotta/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `terracotta/` from the
> `lovelace-background` line in `terracotta.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`terracotta` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `terracotta`, then drag the PNGs in.
...
The theme works without the image. Open `terracotta.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/terracotta.yaml
others ▸  config/themes/terracotta.yaml
```
...
`terracotta.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `terracotta.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Terracotta**.

> If **Terracotta** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/terracotta.yaml` and not in the config root;
...
edit the `lovelace-background` line in `terracotta.yaml` and swap the filename for
`terracotta-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/terracotta/terracotta-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/terracotta/` inside the configuration folder.
