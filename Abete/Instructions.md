# Abete — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro verde abete per Home Assistant, derivato dalla palette **Abete** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dall'ombra della conifera al verde brina, piu quattro accenti
semantici scelti per staccare da una rampa interamente verde.

A fir-green dark theme for Home Assistant, derived from the **Abete**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from conifer shade to frosted green, plus four semantic accents
picked to stand apart from an all-green ramp.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| A0 | `#101d1a` | sfondo app, header, sidebar | app background, header, sidebar |
| A1 | `#1d332d` | superficie delle card | card surface |
| A2 | `#2e4a41` | bordi, divider, disabilitati | borders, dividers, disabled |
| A3 | `#426156` | accent, slider, selezioni | accent, sliders, selections |
| A4 | `#5d7e70` | colore primario, icone attive | primary colour, active icons |
| A5 | `#819f8f` | hover, stati attivi | hover, active states |
| A6 | `#aec6b8` | testo secondario | secondary text |
| A7 | `#e7f0ea` | testo primario | primary text |

**Semantici / Semantic** — `#cf6058` errore·error · `#dda44f` avviso·warning ·
`#9dc45f` successo·success · `#e8c063` luci accese·lights on
...
```
Abete/
├── abete.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── abete-bg.svg                  # sorgente vettoriale / vector source
    ├── abete-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── abete-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `abete` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/abete/abete-bg-2560x1440.png
         homeassistant/www/abete/abete-bg-1170x2532.png

altre  ▸  config/www/abete/abete-bg-2560x1440.png
          config/www/abete/abete-bg-1170x2532.png
```

> La sottocartella `abete` non è facoltativa: il tema punta a
> `/local/abete/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `abete/` dalla
> riga `lovelace-background` in `abete.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `abete` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `abete`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `abete.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #0e1a17 0%, #101d1a 45%, #192c27 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/abete.yaml
altre  ▸  config/themes/abete.yaml
```
...
contenuto di `abete.yaml` e salva con l'icona del dischetto.
...
`abete.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Abete**.

> Se **Abete** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/abete.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `abete.yaml` sostituendo il nome
del file con `abete-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/abete/abete-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/abete/` dentro la cartella di configurazione.
...
create a `abete` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/abete/abete-bg-2560x1440.png
          homeassistant/www/abete/abete-bg-1170x2532.png

others ▸  config/www/abete/abete-bg-2560x1440.png
          config/www/abete/abete-bg-1170x2532.png
```

> The `abete` subfolder is not optional: the theme points at `/local/abete/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `abete/` from the
> `lovelace-background` line in `abete.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`abete` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `abete`, then drag the PNGs in.
...
The theme works without the image. Open `abete.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/abete.yaml
others ▸  config/themes/abete.yaml
```
...
`abete.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `abete.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Abete**.

> If **Abete** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/abete.yaml` and not in the config root;
...
edit the `lovelace-background` line in `abete.yaml` and swap the filename for
`abete-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/abete/abete-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/abete/` inside the configuration folder.
