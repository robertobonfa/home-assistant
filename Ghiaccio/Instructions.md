# Ghiaccio — tema per Home Assistant / Home Assistant theme

![Palette](preview-palette.svg)

Tema scuro blu ghiaccio per Home Assistant, derivato dalla palette **Ghiaccio** di
[wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Otto stop, dal blu abissale all'azzurro brinato, piu quattro accenti
semantici scelti per restare leggibili su fondo blu.

An icy-blue dark theme for Home Assistant, derived from the **Ghiaccio**
palette used on [wallpapers.robertobonfa.com](https://wallpapers.robertobonfa.com).
Eight stops from abyssal blue to frosted pale blue, plus four semantic
accents picked to stay legible against blue.
...
| Token | Hex | Uso | Use |
|---|---|---|---|
| G0 | `#012a4a` | sfondo app, header, sidebar | app background, header, sidebar |
| G1 | `#013a63` | superficie delle card | card surface |
| G2 | `#014f86` | bordi, divider, disabilitati | borders, dividers, disabled |
| G3 | `#2c7da0` | accent, slider, selezioni | accent, sliders, selections |
| G4 | `#468faf` | colore primario, icone attive | primary colour, active icons |
| G5 | `#61a5c2` | hover, stati attivi | hover, active states |
| G6 | `#a9d6e5` | testo secondario | secondary text |
| G7 | `#e8f7fa` | testo primario | primary text |

**Semantici / Semantic** — `#e5586a` errore·error · `#edb44f` avviso·warning ·
`#5fc9a0` successo·success · `#f5d06a` luci accese·lights on
...
```
Ghiaccio/
├── ghiaccio.yaml                        # il tema / the theme
├── Instructions.md                    # questo file / this file
├── preview-palette.svg                # anteprima palette / palette preview
├── changelog.md
├── handoff.md
└── backgrounds/
    ├── ghiaccio-bg.svg                  # sorgente vettoriale / vector source
    ├── ghiaccio-bg-2560x1440.png        # desktop e tablet / desktop and tablet
    ├── ghiaccio-bg-1170x2532.png        # smartphone
...
crea dentro la sottocartella `ghiaccio` e copia i due PNG:

```
HAOS  ▸  homeassistant/www/ghiaccio/ghiaccio-bg-2560x1440.png
         homeassistant/www/ghiaccio/ghiaccio-bg-1170x2532.png

altre  ▸  config/www/ghiaccio/ghiaccio-bg-2560x1440.png
          config/www/ghiaccio/ghiaccio-bg-1170x2532.png
```

> La sottocartella `ghiaccio` non è facoltativa: il tema punta a
> `/local/ghiaccio/…`, dove `/local/` è l'alias pubblico di `www/`. Se metti i PNG
> direttamente in `www/` ottieni un 404. In alternativa togli `ghiaccio/` dalla
> riga `lovelace-background` in `ghiaccio.yaml`.

**Con SFTP o Samba share:** apri la condivisione `config` dal Finder, entra in `www`,
trascina dentro l'intera cartella `ghiaccio` di questo pacchetto. Fine.

**Con Studio Code Server:** apri la cartella `www` nell'albero a sinistra,
tasto destro → *New Folder* → `ghiaccio`, poi trascina i PNG dentro.
...
Il tema funziona anche senza immagine: apri `ghiaccio.yaml` e sostituisci la riga
`lovelace-background` con questa, che usa solo un gradiente CSS e non richiede
nessun file:

```yaml
      lovelace-background: "linear-gradient(180deg, #012541 0%, #012a4a 45%, #01355c 100%)"
```
...
```
HAOS   ▸  homeassistant/themes/ghiaccio.yaml
altre  ▸  config/themes/ghiaccio.yaml
```
...
```
themes/ghiaccio.yaml
```

File editor crea la cartella `themes` da solo. A quel punto incolla dentro il
contenuto di `ghiaccio.yaml` e salva con l'icona del dischetto.
...
`ghiaccio.yaml` dentro quel file rispettando l'indentazione.
...
2. Clicca sul tuo nome in basso a sinistra → **Tema** → seleziona **Ghiaccio**.

> Se **Ghiaccio** non compare nell'elenco — o il profilo utente dice "Nessun tema
...
>    frequente;
> 2. che il file sia in `themes/ghiaccio.yaml` e non nella radice della config;
...
modifica la riga `lovelace-background` in `ghiaccio.yaml` sostituendo il nome
del file con `ghiaccio-bg-1170x2532.png`.

### Se lo sfondo non appare

- Verifica che il file risponda aprendo `http://<tuo-ha>:8123/local/ghiaccio/ghiaccio-bg-2560x1440.png` nel browser. Se ottieni 404, il PNG non è dove pensi: ricontrolla che sia in `www/ghiaccio/` dentro la cartella di configurazione.
...
create a `ghiaccio` subfolder inside and copy both PNGs into it:

```
HAOS   ▸  homeassistant/www/ghiaccio/ghiaccio-bg-2560x1440.png
          homeassistant/www/ghiaccio/ghiaccio-bg-1170x2532.png

others ▸  config/www/ghiaccio/ghiaccio-bg-2560x1440.png
          config/www/ghiaccio/ghiaccio-bg-1170x2532.png
```

> The `ghiaccio` subfolder is not optional: the theme points at `/local/ghiaccio/…`,
> where `/local/` is the public alias for `www/`. PNGs dropped straight into
> `www/` give you a 404. Alternatively, drop `ghiaccio/` from the
> `lovelace-background` line in `ghiaccio.yaml`.

**With SFTP or Samba share:** open the `config` share, go into `www`, drag the whole
`ghiaccio` folder from this package into it. Done.

**With Studio Code Server:** open the `www` folder in the sidebar, right-click →
*New Folder* → `ghiaccio`, then drag the PNGs in.
...
The theme works without the image. Open `ghiaccio.yaml` and replace the
...
```
HAOS   ▸  homeassistant/themes/ghiaccio.yaml
others ▸  config/themes/ghiaccio.yaml
```
...
`ghiaccio.yaml` in and save with the disk icon.
...
If you already use `!include themes.yaml`, paste the contents of `ghiaccio.yaml`
...
2. Click your name in the bottom left → **Theme** → pick **Ghiaccio**.

> If **Ghiaccio** is not in the list — or the profile page says "No theme
...
> 2. that the file sits at `themes/ghiaccio.yaml` and not in the config root;
...
edit the `lovelace-background` line in `ghiaccio.yaml` and swap the filename for
`ghiaccio-bg-1170x2532.png`.

### If the background does not show up

- Check the file is reachable at `http://<your-ha>:8123/local/ghiaccio/ghiaccio-bg-2560x1440.png`. A 404 means the PNG is not where you think — make sure it is in `www/ghiaccio/` inside the configuration folder.
